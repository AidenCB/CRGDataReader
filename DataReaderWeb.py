...
with col1:
    st.image("images/whiteRamapoLogo.png", width=120)

with col2:
    st.markdown("""
    <h2 style='text-align:center; color:white;'>DataReader Web</h2>
    <p style='text-align:center; color:white;'>Created by Aiden Cabrera for the Ramapo Climate Research Group</p>
...
if uploadedFile is not None:
    if st.session_state.show_tool == False:
        ...
        dfRaw = None
        workingDf = None
        st.session_state.dfRaw = dfRaw
        st.session_state.workingDf = workingDf
    elif st.session_state.get('show_tool') == True:
        ...
        if workingDf is None or workingDf.empty:
            if st.session_state.get('show_tool'):
                # Only show this warning if we're past the welcome screen
                st.error("Working data is empty. Please check your file or try uploading again.")
            st.stop() # Stops the script to prevent subsequent AttributeErrors
